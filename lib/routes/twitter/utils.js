...
if (config.twitter.consumer_key && config.twitter.consumer_secret) {
    ...
    let access_tokens = [];
    let access_token_secrets = [];
    let has_access_token = false;
    if (config.twitter.access_token && config.twitter.access_token_secret) {
        access_tokens = config.twitter.access_token.split(',');
        access_token_secrets = config.twitter.access_token_secret.split(',');
    ...
    consumer_keys.forEach((consumer_key, index) => {
        ...
        if (consumer_key && consumer_secret) {
            T[index] = new Twit({
                ...
                access_token,
                access_token_secret,
                app_only_auth: !has_access_token,
            });
            count = index + 1;
        }
    });
